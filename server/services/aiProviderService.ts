  ...
  private async generateOpenAI(request: AIRequest, modelConfig: ModelConfig): Promise<AIResponse> {
    ...
    if (isImageModel) {
      ...
      try {
        ...
        // If organization verification is required, provide demo functionality
        if (error.status === 403 && error.message?.includes('verified')) {
          // Generate a demo image URL for testing purposes
          const demoImageUrl = `https://picsum.photos/1024/1024?random=${Date.now()}`;
          const content = `[DEMO MODE] Image simulation for prompt: "${prompt}". In production, this would be generated by gpt-image-1. Demo URL: ${demoImageUrl}`;
          
          // Continue with demo cost calculation
