...
/**
 * POST /api/picsart/image-upscale
 * Process image upscaling with AI-powered enhancement
 */
router.post('/image-upscale', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🚀 [PICSART] Processing image upscale for user ${userId}`);

    // Validate request body
    const parseResult = imageUpscaleSchema.safeParse(req.body);
    if (!parseResult.success) {
      console.error('❌ [PICSART] Validation failed:', parseResult.error.errors);
      return res.status(400).json({
        success: false,
    ...
    const { imageData, fileName, parameters } = parseResult.data;

    // Check credit balance
    const featureCost = await db.select()
      .from(featureCosts)
      .where(eq(featureCosts.featureName, 'tools.image_upscale'))
      .limit(1);

    if (featureCost.length === 0) {
      return res.status(500).json({
        success: false,
        message: 'Feature cost not configured'
      });
    }

    const costPerUse = featureCost[0].costPerUse;

    // Get user credit balance
    const userResult = await db.select({ creditBalance: users.creditBalance })
      .from(users)
      .where(eq(users.id, userId))
      .limit(1);
    ...
    const userBalance = userResult[0].creditBalance;
    if (userBalance < costPerUse) {
      return res.status(402).json({
        success: false,
        message: `Insufficient credits. Required: ${costPerUse}, Available: ${userBalance}`
      });
    }

    // Deduct credits before processing
    await db.update(users)
      .set({ creditBalance: sql`${users.creditBalance} - ${costPerUse}` })
      .where(eq(users.id, userId));

    console.log(`💳 [PICSART] Deducted ${costPerUse} credits for user ${userId}`);

    try {
      // Process image upscale
      const result = await picsartService.processImageWithUpscale(
        userId,
        imageData,
        fileName,
        parameters
      ...
      await db.insert(aiImgGenerationLogs).values({
        ...
        prompt: `Upscale image: ${fileName} with scale ${parameters.scale || '2'}x`,
        originalImageName: fileName,
        generatedImageUrl: result.processedImageUrl,
        status: 'success',
        cost: costPerUse.toString(),
      ...
      console.log(`✅ [PICSART] Image upscale completed for user ${userId} in ${result.processingTime}ms`);
      ...
      console.log(`🔄 [PICSART] Refunded ${costPerUse} credits to user ${userId} due to processing error`);
    ...
    console.error('❌ [PICSART] Error processing image upscale:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to process image upscale',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
