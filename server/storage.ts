import { 
  ...
  type SupportTicketFile,
  type InsertSupportTicketFile,
...
export class DatabaseStorage implements IStorage {
  ...
  async getSupportTicketById(ticketId: number): Promise<SupportTicketWithRelations | null> {
  ...
  async updateSupportTicket(ticketId: number, updateData: Partial<InsertSupportTicket>): Promise<SupportTicket | null> {
    const [ticket] = await db
      ...
      .set({
        ...
        ...(updateData.status === 'closed' && !updateData.closedAt ? { closedAt: new Date() } : {})
      })
      .where(eq(supportTickets.id, ticketId))
      .returning();
    return ticket || null;
  ...
  // Support Ticket Messages CRUD
  async createSupportTicketMessage(messageData: InsertSupportTicketMessage): Promise<SupportTicketMessage> {
  ...
  async createSupportCategory(categoryData: InsertSupportCategory): Promise<SupportCategory> {
    const [category] = await db
  ...
  // Support Files CRUD
  async createSupportTicketFile(fileData: InsertSupportTicketFile): Promise<SupportTicketFile> {
    const [file] = await db
      .insert(supportTicketFiles)
      .values({
        ...fileData,
        createdAt: new Date()
      })
  ...
  async getSupportTicketFiles(ticketId: number) {
    const files = await db.query.supportTicketFiles.findMany({
      where: eq(supportTicketFiles.ticketId, ticketId),
      with: {
        uploadedBy: true
      },
      orderBy: [asc(supportTicketFiles.createdAt)]
    });
    return files;
  }

  // Dashboard stats for support
  async getSupportStats() {
    ...
    const resolvedTickets = await db.select({ count: count() }).from(supportTickets).where(eq(supportTickets.status, 'resolved'));
    const urgentTickets = await db.select({ count: count() }).from(supportTickets).where(eq(supportTickets.priority, 'urgent'));

    return {
      total: totalTickets[0]?.count || 0,
      open: openTickets[0]?.count || 0,
      resolved: resolvedTickets[0]?.count || 0,
      urgent: urgentTickets[0]?.count || 0
    };
  }
}
