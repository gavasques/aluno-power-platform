...
Crie uma descrição que transforme visitantes em compradores apaixonados pelo produto!`,
  ...
  app.post('/api/products/sample-data', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      
      const sampleProducts = [
        {
          name: 'Kit Cuidados Para Cabelo',
          costItem: '25.50',
          packCost: '2.50',
          weight: '0.8',
          category: 'beauty',
          ...
          ean: '7891234567890',
          brand: 'BeautyPro',
          description: 'Kit completo para cuidados capilares',
          userId: user.id,
          isActive: true
        },
        {
          ...
          ean: '7891234567891',
          brand: 'TechPro',
          description: 'Smartwatch com monitoramento fitness',
          userId: user.id,
          isActive: true
        },
        {
          name: 'Suplemento Whey Protein 900g',
          costItem: '45.00',
          packCost: '3.00',
          weight: '1.2',
          category: 'health',
      ...
      res.status(500).json({ 
        error: 'Erro ao criar produtos de exemplo',
        details: error.message 
      });
    }
  });

  // Calculate pricing for a product in a specific channel
  app.post('/api/products/:productId/channels/:channelType/calculate', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const { productId, channelType } = req.params;
      const { salePrice, customCosts } = req.body;
      ...
      const calculation = await productPricingService.calculatePricing(
        parseInt(productId),
        channelType as any,
  ...
  app.post('/api/products/:productId/channels', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const { productId } = req.params;
      const { channelType, salePrice, customCosts } = req.body;

      if (!channelType || !salePrice) {
        return res.status(400).json({ 
          error: 'Tipo de canal e preço de venda são obrigatórios' 
        });
      }

      const productChannel = await productPricingService.saveProductChannel(
        parseInt(productId),
        user.id,
        channelType,
        parseFloat(salePrice),
      ...
      res.json(productChannel);
    } catch (error: any) {
      console.error('Error saving product channel:', error);
      res.status(500).json({ 
        error: 'Erro ao salvar configuração do canal',
        details: error.message 
      });
    }
  });

  // Get all channels for a product
  app.get('/api/products/:productId/channels', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const { productId } = req.params;

      const channels = await productPricingService.getProductChannels(
        parseInt(productId),
        user.id
      );
      ...
      res.status(500).json({ 
        error: 'Erro ao buscar canais do produto',
        details: error.message 
      });
    }
  });

  // Remove product channel
  app.delete('/api/products/:productId/channels/:channelType', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const { productId, channelType } = req.params;

      await productPricingService.removeProductChannel(
        parseInt(productId),
        user.id,
        channelType as any
      );
      ...
      console.error('Error removing product channel:', error);
      res.status(500).json({ 
        error: 'Erro ao remover canal do produto',
        details: error.message 
      });
    }
  });
  ...
  app.get('/api/user/pricing-settings', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const settings = await productPricingService.getUserSettings(user.id);
  ...
  app.put('/api/user/pricing-settings', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const updatedSettings = await productPricingService.updateUserSettings(
        user.id,
        settings
      );
  ...
  app.get('/api/pricing/channel-types', requireAuth, async (req: Request, res: Response) => {
    try {
      const channelTypes = productPricingService.getAvailableChannelTypes();
      res.json(channelTypes);
    } catch (error: any) {
      console.error('Error fetching channel types:', error);
      res.status(500).json({ 
        error: 'Erro ao buscar tipos de canal',
        details: error.message 
      });
    }
  });
