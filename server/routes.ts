...
Crie uma descrição que transforme visitantes em compradores apaixonados pelo produto!`,
  ...
  // API para exportar departamentos e categorias da Amazon para Excel
  app.get('/api/admin/export-amazon-departments', async (req: Request, res: Response) => {
    try {
      console.log('🔍 [ADMIN_EXPORT] Iniciando exportação de departamentos e categorias Amazon...');

      // 1. Buscar todos os departamentos
      console.log('📋 [DEPARTMENTS] Buscando departamentos...');
      const departmentsResponse = await fetch(
        'https://amazon-data-scraper141.p.rapidapi.com/v1/departments?region=BR',
        {
          headers: {
            'X-RapidAPI-Key': process.env.RAPIDAPI_KEY!,
            'X-RapidAPI-Host': 'amazon-data-scraper141.p.rapidapi.com'
      ...
      if (!departmentsResponse.ok) {
        throw new Error(`Erro ao buscar departamentos: ${departmentsResponse.status}`);
      }
      ...
      // 2. Array para armazenar todos os dados finais
      ...
      // 3. Loop pelos departamentos para buscar categorias
      for (let i = 0; i < departments.length; i++) {
        const dept = departments[i];
        console.log(`🔄 [LOOP ${i + 1}/${departments.length}] Processando departamento: ${dept.name} (${dept.slug})`);

        try {
          // Buscar categorias do departamento atual
          const categoriesResponse = await fetch(
            `https://amazon-data-scraper141.p.rapidapi.com/v1/categories?department_slug=${dept.slug}&region=BR`,
            {
              headers: {
                'X-RapidAPI-Key': process.env.RAPIDAPI_KEY!,
                'X-RapidAPI-Host': 'amazon-data-scraper141.p.rapidapi.com'
          ...
          if (categoriesResponse.ok) {
            ...
            // Processar sub_categories
            const subCategories = categoriesData.data?.sub_categories || [];
            console.log(`  📂 [CATEGORIES] Encontradas ${subCategories.length} categorias`);

            // Adicionar cada categoria ao array final
            subCategories.forEach((cat: any) => {
              exportData.push({
                department: dept.name,
                category: cat.name,
                categoryId: cat.category_id || 'N/A'
              });
            });

            // Se não há sub_categories, adicionar apenas o departamento
            if (subCategories.length === 0) {
              exportData.push({
                department: dept.name,
                category: 'Sem categorias encontradas',
                categoryId: 'N/A'
              });
            }
          } else {
            console.warn(`⚠️  [WARNING] Erro ao buscar categorias para ${dept.name}: ${categoriesResponse.status}`);
            exportData.push({
              department: dept.name,
              category: 'Erro ao carregar categorias',
              categoryId: 'N/A'
            });
          }
          ...
          console.error(`❌ [ERROR] Erro ao processar departamento ${dept.name}:`, error);
          exportData.push({
            department: dept.name,
            category: 'Erro ao processar',
            categoryId: 'N/A'
          });
        }
      }

      console.log(`✅ [EXPORT] Processamento concluído. Total de ${exportData.length} registros`);

      // 4. Retornar os dados para o frontend processar
      res.json({
        success: true,
        data: exportData,
        summary: {
      ...
      console.error('❌ [ADMIN_EXPORT] Erro na exportação:', error);
      res.status(500).json({
        success: false,
        error: 'Erro interno do servidor',
        message: error instanceof Error ? error.message : 'Erro desconhecido'
