...
Crie uma descrição que transforme visitantes em compradores apaixonados pelo produto!`,
  ...
  app.get('/api/products/pricing', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      
      // Fetch products with their channel pricing data
  ...
  // Calculate pricing for a specific channel
  app.post('/api/products/pricing/calculate', requireAuth, async (req: Request, res: Response) => {
    try {
      const { channelType, price, costItem, packCost, commissionPct, adsPct, otherPct, otherValue, taxPct, mlFlexRevenue } = req.body;

      // Validation
      if (!channelType || !price || !costItem) {
        return res.status(400).json({ 
          error: 'Canal, preço e custo do item são obrigatórios' 
        });
      }

      // Import pricing service and calculate
      const { calculateChannelPricing } = await import('./services/pricingService');
      
      const channelInput = {
        price: parseFloat(price),
        costItem: parseFloat(costItem),
        packCost: parseFloat(packCost) || 0,
        commissionPct: parseFloat(commissionPct) || 0,
        adsPct: parseFloat(adsPct) || 0,
        otherPct: parseFloat(otherPct) || 0,
        otherValue: parseFloat(otherValue) || 0,
        taxPct: parseFloat(taxPct) || 0,
        mlFlexRevenue: parseFloat(mlFlexRevenue) || 0
      };

      const result = calculateChannelPricing(channelType, channelInput);
  ...
  // Save pricing configuration for a product
  app.post('/api/products/pricing/save', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const { productId, channelType, price, marginPct, isActive, additionalConfig } = req.body;

      // Validation
      if (!productId || !channelType || !price) {
        return res.status(400).json({ 
          error: 'Produto, canal e preço são obrigatórios' 
        });
      }
      ...
      const existingChannel = await db.query.productChannels.findFirst({
        where: and(
      ...
      if (existingChannel) {
        ...
        const [updatedChannel] = await db.update(productChannels)
          .set({
            price: parseFloat(price),
          ...
          .where(eq(productChannels.id, existingChannel.id))
          .returning();

        res.json(updatedChannel);
      } else {
        // Create new channel
        const [newChannel] = await db.insert(productChannels)
          .values({
            productId,
            channelType,
            price: parseFloat(price),
  ...
  app.delete('/api/products/pricing/:productId/:channelType', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      ...
      await db.delete(productChannels)
        .where(and(
          eq(productChannels.productId, parseInt(productId)),
          eq(productChannels.channelType, channelType)
        ));
  ...
  // Get specific product with pricing channels
  app.get('/api/products/:id/pricing', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
  ...
  // Calculate pricing for specific channel
  app.post('/api/products/pricing/calculate', requireAuth, async (req: Request, res: Response) => {
    try {
      const { channelType, ...input } = req.body;

      if (!channelType || !input.price || !input.costItem) {
        return res.status(400).json({ 
          error: 'channelType, price e costItem são obrigatórios' 
        });
      }

      const { PricingService } = await import('../services/pricingService');
      const result = PricingService.calculateChannelPricing(channelType, input);
  ...
  app.post('/api/products/pricing/target-price', requireAuth, async (req: Request, res: Response) => {
    try {
      const { channelType, targetMarginPct, ...baseCosts } = req.body;

      if (!channelType || !targetMarginPct || !baseCosts.costItem) {
        return res.status(400).json({ 
          error: 'channelType, targetMarginPct e costItem são obrigatórios' 
        });
      }

      const { PricingService } = await import('../services/pricingService');
      const targetPrice = PricingService.calculateTargetPrice(channelType, targetMarginPct, baseCosts);

      res.json({ targetPrice });
    } catch (error: any) {
      console.error('Error calculating target price:', error);
      res.status(500).json({ 
        error: 'Erro no cálculo de preço alvo', 
        details: error.message 
      });
    }
  });
  ...
  app.get('/api/products/pricing/channels/:channelType/config', requireAuth, async (req: Request, res: Response) => {
    try {
      const { channelType } = req.params;

      const { PricingService } = await import('../services/pricingService');
      const config = PricingService.getChannelConfig(channelType as any);

      if (!config) {
        return res.status(404).json({ error: 'Tipo de canal não encontrado' });
      }
      ...
      res.status(500).json({ 
        error: 'Erro ao buscar configuração do canal', 
        details: error.message 
      });
    }
  });

  // Save/update product channel pricing
  app.post('/api/products/:id/channels', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const productId = parseInt(req.params.id);
      ...
      const existingChannel = await db.query.productChannels.findFirst({
        where: and(
          eq(productChannels.productId, productId),
          eq(productChannels.channelType, channelData.channelType)
        )
      });
      ...
      console.error('Error saving product channel:', error);
      res.status(500).json({ 
        error: 'Erro ao salvar canal do produto', 
        details: error.message 
      });
    }
  });
  ...
  app.delete('/api/products/:id/channels/:channelId', requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const productId = parseInt(req.params.id);
      ...
      // Verify channel exists and belongs to product
      const channel = await db.query.productChannels.findFirst({
        where: and(
          eq(productChannels.id, channelId),
          eq(productChannels.productId, productId)
        )
      });
      ...
      await db.delete(productChannels).where(eq(productChannels.id, channelId));

      res.json({ success: true });
    } catch (error: any) {
      console.error('Error deleting product channel:', error);
      res.status(500).json({ 
        error: 'Erro ao deletar canal do produto', 
        details: error.message 
      });
    }
  });

  // Bulk pricing calculation for multiple channels
  app.post('/api/products/pricing/bulk-calculate', requireAuth, async (req: Request, res: Response) => {
    try {
      const { channels, baseCosts } = req.body;

      if (!Array.isArray(channels) || !baseCosts) {
        return res.status(400).json({ 
          error: 'channels (array) e baseCosts são obrigatórios' 
        });
      }

      const { PricingService } = await import('../services/pricingService');
      
      const results = channels.map(channelType => {
        try {
          const calculation = PricingService.calculateChannelPricing(channelType, baseCosts);
          return {
            channelType,
            ...calculation,
            error: null
