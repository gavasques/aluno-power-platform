...
// ========================================
// SIMPLES NACIONAL SIMULATOR TABLES
// ========================================

// Simples Nacional simulations
export const simplesSimulations = pgTable("simples_simulations", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  nomeSimulacao: text("nome_simulacao").notNull(),
  dataCreated: timestamp("data_created").notNull().defaultNow(),
  dataLastModified: timestamp("data_last_modified").notNull().defaultNow(),
}, (table) => ({
  userIdx: index("simples_simulations_user_idx").on(table.userId),
  dateIdx: index("simples_simulations_date_idx").on(table.dataLastModified),
}));

// Monthly simulation data
export const mesSimulacao = pgTable("mes_simulacao", {
  id: serial("id").primaryKey(),
  idSimulacao: integer("id_simulacao").references(() => simplesSimulations.id, { onDelete: 'cascade' }).notNull(),
  competencia: integer("competencia").notNull(), // 1-12
  ano: integer("ano").notNull(),
  faturamento: decimal("faturamento", { precision: 15, scale: 2 }).notNull(),
  anexo: text("anexo").notNull(), // "Anexo I" ou "Anexo II"
  soma12Meses: decimal("soma_12_meses", { precision: 15, scale: 2 }),
  media12Meses: decimal("media_12_meses", { precision: 15, scale: 2 }),
  disponivelMedia: decimal("disponivel_media", { precision: 15, scale: 2 }),
  disponivelAnual: decimal("disponivel_anual", { precision: 15, scale: 2 }),
  aliquotaEfetiva: decimal("aliquota_efetiva", { precision: 5, scale: 4 }),
  valorImposto: decimal("valor_imposto", { precision: 15, scale: 2 })
}, (table) => ({
  simulacaoIdx: index("mes_simulacao_simulacao_idx").on(table.idSimulacao),
  competenciaIdx: index("mes_simulacao_competencia_idx").on(table.competencia, table.ano),
  anexoIdx: index("mes_simulacao_anexo_idx").on(table.anexo),
  uniqueMonthSimulation: unique("unique_month_simulation").on(table.idSimulacao, table.competencia, table.ano),
}));

// Tax bracket table for Simples Nacional calculation
export const faixaAliquota = pgTable("faixa_aliquota", {
  id: serial("id").primaryKey(),
  anexo: text("anexo").notNull(), // "Anexo I" ou "Anexo II"
  faixaInicial: decimal("faixa_inicial", { precision: 15, scale: 2 }).notNull(),
  faixaFinal: decimal("faixa_final", { precision: 15, scale: 2 }).notNull(),
  aliquotaNominal: decimal("aliquota_nominal", { precision: 5, scale: 4 }).notNull(),
  valorDeduzir: decimal("valor_deduzir", { precision: 15, scale: 2 }).notNull()
}, (table) => ({
  anexoIdx: index("faixa_aliquota_anexo_idx").on(table.anexo),
  faixaIdx: index("faixa_aliquota_faixa_idx").on(table.faixaInicial, table.faixaFinal),
}));

// Simples Nacional Relations
export const simplesSimulationsRelations = relations(simplesSimulations, ({ one, many }) => ({
  user: one(users, {
    fields: [simplesSimulations.userId],
    references: [users.id],
  }),
  meses: many(mesSimulacao),
}));

export const mesSimulacaoRelations = relations(mesSimulacao, ({ one }) => ({
  simulacao: one(simplesSimulations, {
    fields: [mesSimulacao.idSimulacao],
    references: [simplesSimulations.id],
  }),
}));

// Simples Nacional Types
export const insertSimplesSimulationSchema = createInsertSchema(simplesSimulations).omit({
  id: true,
  dataCreated: true,
  dataLastModified: true,
});

export const insertMesSimulacaoSchema = createInsertSchema(mesSimulacao).omit({
  id: true,
  soma12Meses: true,
  media12Meses: true,
  disponivelMedia: true,
  disponivelAnual: true,
  aliquotaEfetiva: true,
  valorImposto: true
});

export const insertFaixaAliquotaSchema = createInsertSchema(faixaAliquota).omit({
  id: true,
});

export type SimplesSimulation = typeof simplesSimulations.$inferSelect;
export type InsertSimplesSimulation = z.infer<typeof insertSimplesSimulationSchema>;
export type MesSimulacao = typeof mesSimulacao.$inferSelect;
export type InsertMesSimulacao = z.infer<typeof insertMesSimulacaoSchema>;
export type FaixaAliquota = typeof faixaAliquota.$inferSelect;
export type InsertFaixaAliquota = z.infer<typeof insertFaixaAliquotaSchema>;

// Simples simulation with all months
export type SimplesSimulationWithMeses = SimplesSimulation & {
  meses: MesSimulacao[];
};
